    ...
    methods:{
        ...
        Dilation(pointX, pointY){
            ...
            for(let j=pointY; j<this.canvas2.height; j++){
                ...
                if(element.data[(2-1)*4] == 255||element.data[(4-1)*4] == 255||element.data[(5-1)*4] == 255||element.data[(6-1)*4] == 255||element.data[(8-1)*4] == 255){
        ...
        ORgate(){
            let imgData2 = this.ctx2.getImageData(0, 0, this.canvas2.width, this.canvas2.height)
            let imgData3 = this.ctx3.getImageData(0, 0, this.canvas3.width, this.canvas3.height)
        ...
        renegative(){
            let imgData = this.ctx4.getImageData(0, 0, this.canvas.width, this.canvas.height)
            //*****************轉負片順便二值化********************
            for(let i=0; i<imgData.data.length; i+=4){
                imgData.data[i] = (255 - imgData.data[i]) > 128 ? 255 : 0
                imgData.data[i+1] = (255 - imgData.data[i+1]) > 128 ? 255 : 0
